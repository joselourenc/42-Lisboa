...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/11 16:27:16 by jsilva-d          #+#    #+#             */
/*   Updated: 2021/03/22 16:14:27 by jsilva-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line_bonus.h"

int			read_file_until_nl(t_str **str, int fd)
{
	char	*buffer;
	int		r;
...
int			malloc_next_line(t_str **str, char **line)
{
	t_str	*first;
	int		i;
	int		j;
...
int			write_next_line(t_str **str, char **line)
{
	int		idx[2];
	int		remaining;
	t_str	*next;
...
static int	free_all(t_fd **list, int fd, char *buf)
{
	t_fd	*first;
	t_fd	*lt[2];
...
int			get_next_line(int fd, char **line)
{
	...
	if ((buffer = NULL) || (!read_rem && current->str))
		read_rem = !find_n1(current->str, NULL);
